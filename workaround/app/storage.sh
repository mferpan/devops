...
function createStorage {
  ...
  [ $? -ne 0 ] && error "There is NO volgroup with FREE available" && exit 1
  ...
  if [[ $DISK_SIZE_MB -gt $VG_FREE_SPACE ]]; then
    ...
    $LVCREATE -L $DISK_SIZE""M -n $USER_ID $VOLGROUP
